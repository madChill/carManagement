...
import { getCars, deleteUser, getUsers } from '../../utils/request'
...
export const uselogicForm = () => {
    ...
    const [scans, setscans] = useState<IUser[]>([]);
    const [total, settotal] = useState<number>(0);
    const [search, setSearch] = useState<IUser>();
    const setsearch = debounce(val => setSearch({...search, ...val}), 1000);
    useEffect(() => {
        setloading(true)
        getUsers({...query, ...search}).then((rawData: AxiosResponse<IGetResult<IUser>>) => {
